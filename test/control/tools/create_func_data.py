...
default_data = {'value': (1.00, 'float')}
default_condition_data = {'condition': ([(u'判断输入信号', [], {'condition_value': u'端口0', 'operation_value': u'有信号'})], 'list'), 'check_allconditions': True}
default_none_data = {}
default_time_data = {'time': (1.0, 'float')}
default_value_data = {'value': (1, 'int')}
...
def rewrite_funcs_data():
    ...
    with open(os.path.abspath(os.path.join(current_path, '../reference.yml')), 'r') as f1:
        ...
        _demos_control_path = os.path.join(references['demos_path'], 'luaprogramme/control/funcs_data.yml')
        with open(os.path.abspath(_demos_control_path), 'w') as f1:
            f1.write(yaml.dump(_data))
            f1.close()
